...
def curve_fit_patched(f, xdata, ydata, p0=None, sigma=None, absolute_sigma=False, check_finite=True, **kw):
	...
	if 'absolute_sigma' in args and 'check_finite' in args and not force_patch:
		...
		if absolute_sigma: # invert the normalization done by curve_fit
			popt = rt[0]
			s_sq = sum(((asarray(ydata) - f(asarray(xdata), *popt)) / (asarray(sigma) if sigma != None else 1.0)) ** 2) / (len(ydata) - 1)
			pcov = rt[1] / s_sq
			rt = (rt[0], pcov) + rt[2:]
	return rt
