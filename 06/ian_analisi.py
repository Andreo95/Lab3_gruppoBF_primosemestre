...
dphase[dphase < 1/1e3] += 2*1*np.sqrt(dt**2 * freq**2)[dphase < 1/1e3]

def sfasinteg(f, ft):
    return np.arctan(-f/ft)/np.pi

# sfasinteg.mask = freq >100
...
sfasderiv.mask = (freq < 3e3) #& (freq > 30)
sfasderiv.deriv = lambda f, ft: -2*ft/f**2/(1 + (ft/f)**2)/np.pi
