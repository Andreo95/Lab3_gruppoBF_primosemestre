...
#Load txt
Y = loadtxt("Desktop/1polar.txt", unpack = True)
X = linspace(0,184*pi/180,24)
dX= array([0.25*pi/180]*24)
dY= mme(Y/10**6,"ampere")*10**6
#Define fit function
def f(X, a, b, c):
...
clf()
errorbar(X*180/pi,Y,dY,dX*180/pi, fmt=".",capsize=0)
xlabel("Angolo in gradi [°]")
ylabel("Intensità di corrente [$\mu A$]")
title("Grafico corrente vs inclinazione")
l=linspace(min(X),max(X),1000)
plot(l*180/pi,f(l,*par))

chi = sum((Y-f(X,*par))**2/(dY**2+(df(X,*par)*dX)**2))
print("chi,ndof,chi/ndof=",chi,len(X)-len(par),chi/(len(X)-len(par)))
sigma=sqrt(diag(cov))
c = zeros((3,3))
for i in range(3):
    for j in range(3):
        c[i,j]=cov[i, j]/(sigma[i]*sigma[j])
par[1]*=180/pi
sigma[1]*=180/pi
for i in range(len(par)):
    print("p = %s" % xe(par[i],sigma[i]))
print(c)
